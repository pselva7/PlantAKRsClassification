...
## ❗️create a directory and copy your input_ProteinSequence_file, RefPlantAKRs.fa, and PlantAKRsClassification.sh in the directory.
...
## ❗️check line 26 and input the name of your input_ProteinSequence_file. 
...
input_ProteinSequence_file=$"TaAKRs.fa"	# ❗️Change the file name TaAKRs.fa with your file name
...
cp RefPlantAKRs.fa Classification/RefPlantAKRs
...
Note1: If you installed seqkit under conda, first activate conda in the terminal using the command conda activate and run ./PlantAKRsClassification.sh
Note2: If you installed seqkit outside conda environment, make sure you export the seqkit path to ~/.bashrc\n then run ./PlantAKRsClassification.sh.\n"
    cd ..    
    rm -r Classification
    exit
fi
...
# Step_4: make a protein database using RefPlantAKRs
makeblastdb -in RefPlantAKRs -dbtype prot

## exit if ncbi-blast+ is not set properly
if ! [ -s *.phr ]; then
   echo -e "\nncbi-blat+ is not in the script running environment. Aborting\n
Note1: If you installed ncbi-blat+ under conda, first activate conda in the terminal using the command conda activate and run ./PlantAKRsClassification.sh
...
if [ -s *.phr ]; then
echo -e "\n\nA protein database of RefPlantAKRs has been created successfully"
fi

# Step_5: query the input sequences against the RefPlantAKRs database via blastp  search
blastp -query 3.${input_ProteinSequence_file%.*}".fa1" -db RefPlantAKRs -max_target_seqs $max_target_seqs -num_threads $threads -outfmt $outfmt -out 4.${input_ProteinSequence_file%.*}".blastout"
...
Note: For Linux operating system, install datamash using the command sudo apt install datamash and run ./PlantAKRsClassification.sh\n"
    cd ..    
    rm -r Classification
    exit
fi
...
awk -v SubPer=$SubPer '$3 >= SubPer {print $1"\t"$2}' 7.${input_ProteinSequence_file%.*}".blastout3" OFS="\t" | awk -F'\t' -vOFS='\t' '{ gsub("AKR-", "", $2) ; print $1, $2, $3=substr($2, 1, length($2)-1), $4="AKR"}' | awk -F'\t' -vOFS='\t' '{ print $1, $4, $3, $2 }' > 8.${input_ProteinSequence_file%.*}"ClassifiedSum1"
...
awk -v protein_identity2=$protein_identity2 -v query_coverage2=$query_coverage2 -v alignment_length2=$alignment_length2 '{if( ($8 >= protein_identity2 && $11 >= query_coverage2) || ($8 >= protein_identity2 && $11 < query_coverage2 && $9 >= alignment_length2)) { print } }' OFS="\t" 11.${input_ProteinSequence_file%.*}"UnC1" | awk -F'\t' -vOFS='\t' '{ gsub("AKR-", "", $5) ; print $1, $2=substr($5, 1, length($5)-1)}' | sort | datamash -s crosstab 1,2 > 12.${input_ProteinSequence_file%.*}"UnC2"
...
awk -v SubPer=$SubPer '$3 >= SubPer {print $1"\t"$2}' 13.${input_ProteinSequence_file%.*}"UnC3" OFS="\t" | awk -F'\t' -vOFS='\t' '{print $1, $2, $3="AKR", $4="UnC"}' | awk -F'\t' -vOFS='\t' '{ print $1, $3, $2, $4 }' > 15.${input_ProteinSequence_file%.*}"ClassifiedSum2"
...
awk -F, 'FNR==NR {f2[$1];next} !($0 in f2)' 16.${input_ProteinSequence_file%.*}"ClassifiedGeneIDsSum" 1.${input_ProteinSequence_file%.*}"GeneIDs" | awk -F'\t' -vOFS='\t' '{print $1, $2="Non-AKR", $3="UnC", $4="UnC"}' > 17.${input_ProteinSequence_file%.*}"UnclassifiedSum"
...
stat4=`sed '/Non-AKR/d' 18.${input_ProteinSequence_file%.*}"SubgroupInfo" | awk '{A[$3]++}END{for(i in A)print i,A[i]}' | sort | awk '{sum+=$2;}END{print sum;}'`
stat5=`sed '/Non-AKR/d' 18.${input_ProteinSequence_file%.*}"SubgroupInfo" | awk '{A[$3]++}END{for(i in A)print i,A[i]}' | sort | awk 'BEGIN {RS=""}{gsub(/\n/,"; ",$0); print $0}'`
...
if [ -z "$stat3" ]; then
    echo -e "There were $stat1 seqeunces in the input query file. No Non-AKRs were detected.\n"
else
    echo -e "There were $stat1 seqeunces in the input query file that includes $stat2 AKRs and $stat3 Non-AKR(s).\n"
fi

# to know the composition of groups assigned to input sequences
if test $stat2 = $stat4; then
    echo -e "Group was assigned for all AKR sequences present in the input query file: $stat5.\n"
else
    echo -e "Group was assigned for $stat4 of $stat2 AKR sequences present in the input query file: $stat5.\n"
fi

# to know the composition of subgroups assigned to input sequences
if test $stat2 = $stat6; then
    echo -e "Subgroup was assigned for all AKR sequences present in the input query file: $stat7.\n"
else
    echo -e "Subgroup was assigned for $stat6 of $stat2 AKR sequences present in the input query file: $stat7.\n
You can keep the subgroup unclassified AKR(s) (n = $stat8) as such or assign them subgroup based on further validation using any other logical criteria (e.g. synteny, phylogeny or relaxed homology condition)."
fi

# what to do next
if [ ! -z "$stat3" ]; then
    echo -e "\nVerify whether the Non-AKRs (n = $stat3) have AKR domain by any means (e.g. verify them in Pfam database: http://pfam.xfam.org/). If they do have AKR domain(s), consider them as group UnC AKRs (i.e. group Unclassified AKRs). You can assign group to the group UnC AKR(s) based on further validation using any other logical criteria (e.g. synteny, phylogeny or relaxed homology condition). After domain check, you can exclude all the Non-AKRs (which would have no AKR domain, no group information and no subgroup information) for all further downstream processess as per your experiment design.\n"
else
    echo -e ""
fi
