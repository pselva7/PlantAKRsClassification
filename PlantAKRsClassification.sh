...
# Step_20: Prepare the final summary
...
# Step_21: Prepare the final stats/notes
...
Following are some quick stats:"
    ...
    echo -e "\n### Something went wrong. Do not use the final output. Check the intermediate files to trace the bug(s) ###\n"
fi

## to get some stats
stat1=`awk 'END {print NR}' 18.${input_ProteinSequence_file%.*}"SubgroupInfo"`
stat2=`awk '{A[$2]++}END{for(i in A)print A[i],i}' 18.${input_ProteinSequence_file%.*}"SubgroupInfo" | sort | awk 'FNR == 1 {print $1}'`
stat3=`awk '{A[$2]++}END{for(i in A)print A[i],i}' 18.${input_ProteinSequence_file%.*}"SubgroupInfo" | sort | awk 'FNR == 2 {print $1}'`
